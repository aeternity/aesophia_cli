...
## Create calldata
./aesophia_cli --create_calldata test/contracts/identity.aes --call "id(42)" -o ${TMPFILE}.calldata1
...
## Decode data
RES=`./aesophia_cli -b aevm test/contracts/identity.aes --call_result cb_AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAACr8s/aY --call_result_fun id`
echo -e "${RES}"
RES=`echo "${RES}" | tail -n1`
if [ "${RES}" != "42" ]; then
